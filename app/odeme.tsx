...
function OrderSuccess({
  orderNo, isAllDigital, onContinue,
}: { orderNo: string; isAllDigital: boolean; onContinue: () => void }) {
  return (
    <View style={styles.successContainer}>
      <Text style={styles.successEmoji}>✅</Text>
      <Text style={styles.successTitle}>Siparişiniz Alındı!</Text>
      <Text style={styles.successSub}>Sipariş No: <Text style={{ color: C.text, fontWeight: "700" }}>#{orderNo}</Text></Text>
      <Text style={styles.successDesc}>
        {isAllDigital
          ? "Dijital dosyalarınız hesabınıza eklendi. Kütüphanenizden indirebilirsiniz."
      ...
      <TouchableOpacity style={[styles.primaryBtn, { marginTop: 24, width: "100%" }]} onPress={onContinue}>
        <Text style={styles.primaryBtnText}>Siparişlerimi Gör</Text>
      </TouchableOpacity>
    </View>
  );
}
  ...
  if (orderNo) {
    return (
      <View style={{ flex: 1, backgroundColor: C.background }}>
        ...
        <WizardHeader title="Sipariş Onayı" onBack={() => router.replace("/(tabs)")} />
        <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: "center", padding: 16 }}>
          <OrderSuccess
            orderNo={orderNo}
            isAllDigital={isAllDigital}
            onContinue={() => router.replace("/(tabs)/orders")}
          />
        </ScrollView>
      </View>
    );
...
const styles = StyleSheet.create({
  ...
  successSub:       { fontSize: 14, color: C.text2, marginBottom: 8 },
  successDesc:      { fontSize: 13, color: C.text2, textAlign: "center", lineHeight: 20, maxWidth: 280 },
  digitalNote:      { marginTop: 16, backgroundColor: "rgba(59,130,246,0.06)", borderWidth: 1, borderColor: "rgba(59,130,246,0.15)", borderRadius: 12, padding: 12 },
