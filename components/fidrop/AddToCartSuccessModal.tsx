    ...
    <Modal visible={visible} transparent animationType="none" statusBarTranslucent>
      <Animated.View entering={FadeIn.duration(200)} style={s.overlay}>
        ...
        >
          ...
          <Text style={s.title}>Sepete Eklendi!</Text>
          ...
          <View style={s.trustBox}>
            ...
            <Text style={s.trustText}>
              Dosyanız{" "}
              <Text style={s.trustBold}>Fimarkt mühendisleri</Text>
              {" "}tarafından kalite kontrolünden geçirilip, standartlarımıza uyan{" "}
              <Text style={s.trustBold}>onaylı Çözüm Ortağımıza</Text>
          ...
          {totalPrice != null && (
            <View style={s.priceRow}>
              <Text style={s.priceLabel}>Sepet toplamı güncellendi</Text>
              <Text style={s.priceValue}>₺{totalPrice.toLocaleString("tr-TR")}</Text>
            </View>
          )}

          {/* CTA'ler */}
          <TouchableOpacity style={s.primaryBtn} onPress={handleGoToCart} activeOpacity={0.85}>
            <Text style={s.primaryBtnText}>🛒  Sepete Git →</Text>
          </TouchableOpacity>
          <TouchableOpacity style={s.secondaryBtn} onPress={onNewModel} activeOpacity={0.85}>
            <Text style={s.secondaryBtnText}>+ Başka Model Yükle</Text>
          </TouchableOpacity>
